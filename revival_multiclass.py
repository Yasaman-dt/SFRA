...
@torch.no_grad()
def _sample_predicted_as_class_with_probs(
    W, b, emb_dim, target_class, per_class_strict=1000, batch=4096, device="cuda"
):
    """
    Like _sample_predicted_as_class(...), but also returns the FULL probs rows
    for the matched items so we can inspect confidence on multiple forget classes.
    """
    feats_buf = []
    probs_buf = []
    ...
    while sum(t.shape[0] for t in feats_buf) < per_class_strict:
        ...
        if mask.any():
            ...
            probs_buf.append(probs[mask])   # full [num_classes] row
    ...
    return feats_cat, probs_cat  # feats [M, D], probs [M, C]
...
def build_synthetic_embeddings_and_splits_multi(
    ...
    - Retain set: same as single-class (top-K per retain class by own confidence).
    - Forget set: for EACH retain class r, take the *lowest* confidence-on-r pool,
      then split it *evenly* across all forget classes by ranking their probs.
    """
    assert len(forget_classes) >= 2, "Use the single-class builder for 1 forget class."
    ...
    # -------- Retain split: predicted-as-own-class then top-K by confidence --------
    ...
    # -------- Forget split: balanced per retain class across forget classes --------
    forget_feats_chunks = []
    forget_labels_chunks = []

    kF = len(forget_classes)
    quota_per_forget = per_retain_for_each_forget  # per FORGET per retain class
    needed_per_retain_class = kF * quota_per_forget

    for r in retain_classes:
        # pool that are predicted as r (so we can rank by "how weakly they belong to r")
        feats_r, probs_r_full = _sample_predicted_as_class_with_probs(
        ...
        chosen_mask = torch.zeros(pool_idx.numel(), dtype=torch.bool, device=device)

        # assign evenly across forget classes, greedily by each forget's prob
        for f in forget_classes:
            ...
            forget_labels_chunks.append(torch.full((sel_abs.numel(),), f, device=device, dtype=torch.long))

        # If we couldn’t fill all quotas (rare), we stop early for this retain class.

    if forget_feats_chunks:
        forget_feats  = torch.cat(forget_feats_chunks,  dim=0)
        forget_labels = torch.cat(forget_labels_chunks, dim=0)
    else:
        forget_feats  = torch.empty(0, emb_dim, device=device)
        forget_labels = torch.empty(0, dtype=torch.long, device=device)

    # --- build loaders on CPU tensors ---
    ...
    # per-forget counts (useful sanity check)
    per_forget_counts = {}
    for f in forget_classes:
        per_forget_counts[int(f)] = int((forget_labels == f).sum().item())
...
# Pretty tag and experiment dir
if len(forget_classes) == 1:
    tag_forget = f"{forget_classes[0]}"
else:
    tag_forget = "multi_" + "_".join(str(c) for c in forget_classes)
...
forget_spec = forget_classes[0] if len(forget_classes) == 1 else forget_classes
ckpt = checkpoint_for(method, dataset_name, model_name, forget_spec, lr, DIR)
...
(
    ...
    trainset, testset, forget_classes,             
...
if len(forget_classes) == 1:
    ...
    synth = build_synthetic_embeddings_and_splits_multi(
        model=model,
        num_classes=num_classes,
        forget_classes=forget_classes,
        device=device,
...
if best["row"] is not None:
    ...
    if len(forget_classes) == 1:
        ...
        print(f"[BEST] No best row found for forget={forget_classes}.")
