...
parser.add_argument(
    '--forget_threshold', type=float, default=0.30,
    help='Only samples with p(class)<forget_threshold are eligible for the forget pool (per retain class).'
)
parser.add_argument(
...
def append_best_for_class(
    ...
    """
    Append exactly ONE row per forget_class, with your best epoch's metrics
    + configuration summary (retain_per_class, totals).
    """
    if best_row is None:
    ...
    row = {
        ...
        **best_row,  # expects keys like epoch, syn_* , all_*, train_*, test_*, RS
    }
...
def _collect_for_class(
    ...
    """
    Keep sampling embeddings until we have:
      - need_top items with largest p(target_class)  (retain)
      - need_bottom items with p(target_class) < bottom_thresh, smallest first (forget)
    All samples are *predicted as target_class* by (W,b).
    Returns: (top_feats, top_probs), (bottom_feats, bottom_probs), stats dict
    """
    feats_all   = []
    probs_all   = []
    tries = 0
    ...
    while True:
        ...
        if mask.any():
            ...
            probs_all.append(probs[mask, target_class])

        # evaluate progress every iteration
        cat_feats = torch.cat(feats_all, dim=0) if feats_all else torch.empty(0, emb_dim, device=device)
        cat_probs = torch.cat(probs_all, dim=0) if probs_all else torch.empty(0, device=device)
        ...
        tries += 1
        if tries >= max_tries:
            # fall back: give whatever we have (warn via stats)
            if have == 0:
                # nothing at all predicted as this class
                return (torch.empty(0, emb_dim, device=device), torch.empty(0, device=device)), \
            ...
            if under.numel() > 0:
                under_probs = cat_probs.index_select(0, under)
                _, under_sorted_asc = torch.sort(under_probs, descending=False)
                bottom_idx = under.index_select(0, under_sorted_asc[:min(need_bottom, under_sorted_asc.numel())])
            else:
                bottom_idx = torch.empty(0, dtype=torch.long, device=device)

            return (
                (cat_feats.index_select(0, top_idx),    cat_probs.index_select(0, top_idx)),
                (cat_feats.index_select(0, bottom_idx), cat_probs.index_select(0, bottom_idx)),
                {
                    "sampled_total_pred_as_class": int(have),
                    "bottom_candidates_under_thresh": int(under.numel()) if under is not None else 0,
                    "warning": "max_tries reached; bottom/top may be undersized"
                }
            )
...
def build_synthetic_embeddings_and_splits(
    ...
    For each retain class c != forget_class:
      - sample until we have retain_top_k (most confident) predicted-as-c
      - sample until we have per_retain_for_forget bottom candidates predicted-as-c
        with p_c < forget_threshold, then relabel them as forget_class
    """
    ...
    forget_feats_list, forget_labels_list = [], []

    stats_per_class = {}

    for c in retain_classes:
        (top_feats, top_probs), (bot_feats, bot_probs), stats = _collect_for_class(
        ...
        # retain: label as their own class c
        if top_feats.numel() > 0:
            retain_feats_list.append(top_feats)
            retain_labels_list.append(torch.full((top_feats.shape[0],), c, device=device, dtype=torch.long))

        # forget: relabel AS the chosen forget_class
        if bot_feats.numel() > 0:
            forget_feats_list.append(bot_feats)
            forget_labels_list.append(torch.full((bot_feats.shape[0],), forget_class, device=device, dtype=torch.long))

        stats_per_class[c] = {
            **stats,
            "retained_kept": int(top_feats.shape[0]),
            "forgot_kept": int(bot_feats.shape[0]),
        }
    ...
    forget_labels = torch.cat(forget_labels_list, dim=0) if forget_labels_list else torch.empty(0, dtype=torch.long, device=device)

    # CPU loaders for FC-only training
    retain_ds = TensorDataset(retain_feats.cpu(), retain_labels.cpu())
    forget_ds = TensorDataset(forget_feats.cpu(), forget_labels.cpu())

    # If a dataset is empty, keep shuffle=False so DataLoader uses SequentialSampler (works with len==0)
    retain_loader = DataLoader(
        retain_ds, batch_size=loader_batch_size, shuffle=(len(retain_ds) > 0), drop_last=False
    )
    forget_loader = DataLoader(
        forget_ds, batch_size=loader_batch_size, shuffle=(len(forget_ds) > 0), drop_last=False
    )
    ...
    summary = {
        ...
        "retain_top_k_per_class": retain_top_k,
        ...
        "per_class_stats": {int(k): v for k, v in stats_per_class.items()},
    }
    return {
...
for forget_class in forget_classes:
    ...
    train_feats  = torch.cat([synth["retain_feats"], synth["forget_feats"]], dim=0).cpu()
    train_labels = torch.cat([synth["retain_labels"], synth["forget_labels"]], dim=0).cpu()
    ...
    if best["row"] is not None:
        retain_per_class = retain_top_k  # requested per-class retain count
